    ...
    def get_neighbors(self, node: Node) -> List[Node]:
        ...
        for dx, dy in directions:
            ...
            if self.is_valid_position(new_x, new_y):
                ...
                print(f"Found valid neighbor: ({new_x}, {new_y}) - Direction: {direction_names[directions.index((dx, dy))]}")
    ...
    def dfs(self, start: Tuple[int, int], goal: Tuple[int, int]) -> Optional[List[Tuple[int, int]]]:
        ...
        while stack:
            ...
            if current_pos == goal:
                ...
                print(f"Complete path: {' -> '.join(str(p) for p in final_path)}")
                return final_path
            ...
            if current_pos not in visited:
                ...
                # Komşuları kontrol et ve geçerli olanları ekle
                for neighbor in neighbors:
                    neighbor_pos = (neighbor.x, neighbor.y)
                    if neighbor_pos not in visited:
                        valid_neighbors.append(neighbor)
                
                print(f"Valid unvisited neighbors: {[(n.x, n.y) for n in valid_neighbors]}")
                
                # Komşuları öncelik sırasına göre sırala
                # Hedef yönündeki komşulara öncelik ver
             #   valid_neighbors.sort(key=lambda n: abs(n.x - goal[0]) + abs(n.y - goal[1]))
                
                # Her komşuyu stack'e ekle
                for neighbor in reversed(valid_neighbors):
                    new_path = path + [current_pos]
                    stack.append((neighbor, new_path))
                    print(f"Adding {(neighbor.x, neighbor.y)} to stack")
